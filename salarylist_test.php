    ...
    <div class="card border-0 bg-light shadow-sm pb-2">
        ...
        if(isset($salaryID))
        {
            if ($conn->connect_error) {
                ...
                $sql = "SELECT * from salary join staff on salary.staffID=salary.staffID where salary.salaryID=$salaryID";
                $result = $conn->query($sql);
                $count = $result->num_rows;
                $i = 0;
                $row = mysqli_fetch_assoc($result);
                if ($count > 0) {
                    $age = date("Y") - $row['payDay'];
                    ...
                    echo "<div class='col-6 py-1'>".$row['salary']."</div>";
                    echo "</div>";
                    echo "<div class='row border-bottom'>";
                    echo "<div class='col-6 py-1 text-right border-right'><strong>Address</strong></div>";
                    echo "<div class='col-6 py-1'>".$row['payStatus']."</div>";
                    echo "</div>";
                }
            }
        }
